    ...
    'app.ast $ %{} 'FileEntry
      :defs $ {}
                    ...
                    let
                        ...
                        rules $ if
                          ...
                          .slice
                            ...
                            , 1
                          []
                        defs $ reel.schema/read-field file :defs
        ...
        'match-references $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn match-references (form ns-dict defs-dict defaults-dict current-defs current-ns) (js/console.log |Defaults defaults-dict)
            let
        ...
        'tree->cirru $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn tree->cirru (x)
            if (&struct:matches? x schema/CirruLeaf)
              reel.schema/read-field x :text
              ->
                reel.schema/read-field x :data
                .to-list
                .sort-by first
                map $ fn (entry)
          ...
          :code $ quote $ defcomp comp-container (reel)
            let
              ...
              div
                ...
                case-default
                  reel.schema/read-field store :page
                  <> "|Unknown route"
                  :viewer $ div
                    ...
                    if
                      ...
                      if
                        ...
                        <> |Nothing
                  :textarea $ comp-text-area
                    reel.schema/read-field store :text
                    reel.schema/read-field store :error
                  :input $ comp-file-input $ reel.schema/read-field store :error
                  :about $ comp-about
                  :graph $ comp-graph (>> states :graph)
                    reel.schema/read-field store :calcit
                comp-messages
          ...
          :code $ quote $ defn on-file-change (e d!)
            let
                target $ unsafe-coerce
                  ...
                  , 'JsObject
                files $ unsafe-coerce (.-files target) 'List
                file $ unsafe-coerce (aget files 0) 'JsObject
              if (js-present? file)
                if
                  not=
                    unsafe-coerce (.-name file) 'String
                    , |calcit.cirru
                  do $ d! :error $ str "|Expected calcit.cirru , but got "
                    unsafe-coerce (.-name file) 'String
                  let
                      fr $ new js/FileReader
                    set! (.-onload fr)
                      fn (event)
                        d! :load/calcit $ parse-cirru-edn
                          unsafe-coerce
            ...
            list->
              {}
                ...
                :style $ if last?
                  ...
                  if
                    every?
                      reel.schema/read-field expr :data
                      fn (entry)
                        option:unwrap-or
                          option:map (last entry)
                            fn (node) (&struct:matches? node schema/CirruLeaf)
                          , false
                  ...
                  let-sugar
                    ...
                    [] k $ if (&struct:matches? child schema/CirruExpr)
                      ...
                      div
                        ...
                        <>
                          str $ ->
                            reel.schema/read-field child :at
              ...
              div ({})
                ...
                list->
                  {} $ :style $ {}
                    ...
                    :margin-bottom 2
                  ->
                    reel.schema/read-field file-info :defs
                    .to-list
                    map $ fn (entry)
                      let-sugar
                            [] def-name def-info
                        ...
                        [] def-name $ div
                          ...
                          div
                            ...
                            =< 8 nil
                            comp-md-block
                              reel.schema/read-field def-info :doc
                              {} $ :class-name css/font-normal
                          comp-expr
                            reel.schema/read-field def-info :code
                            , false
          :examples $ []
          :schema $ :: 'Fn $ {}
            :return 'respo.schema/Component
        ...
        'comp-graph $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-graph (states snapshot)
            let
                ...
                entry $ .split
                  reel.schema/read-field state :init-fn
                  , |/
              div
                ...
                div
                  ...
                  button $ {} (:class-name css/button) (:inner-text |Button)
                    :on-click $ fn (e d!) (; js/console.log snapshot)
                      d! cursor $ assoc state :graph $ build-deps-graph (nth entry 0) (nth entry 1)
                ...
                div
                  ...
                  if
                    ...
                    comp-graph-tree (nth entry 0) (nth entry 1)
                        ...
                        , & $ -> deps $ map
                          fn (dep)
                            if
                              ...
                              comp-graph-tree
                                reel.schema/read-field dep :ns
                                reel.schema/read-field dep :def
                                , dict (.include footprints path) files
                      div
          ...
          :code $ quote $ defcomp comp-viewer (calcit)
            div ({})
              ...
              div
                ...
                list-> ({})
                  ->
                    reel.schema/read-field calcit :files
                    .to-list
                    map $ fn (entry)
                      let-sugar
                            [] filename file-info
    ...
    'app.schema $ %{} 'FileEntry
      :defs $ {}
        ...
        'StoreData $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defstruct StoreData (:states 'Map) (:error 'Dynamic) (:calcit 'Dynamic) (:page 'Tag) (:text 'String) (:messages 'Map)
          :examples $ []
          :schema $ :: 'Enum
        'store $ %{} 'CodeEntry (:doc |)
